...
void cargarmultiple(char [][9], int ,int , int , int );
...
void mainloop(char asientos[][9], int butacas[]){
    ...
    while(f != -1){
        ...
        if(asientos[f-1][posc] == 'r'){
            ...
            cant = validar(1, 9, 8);
            if(cant!= 1){
                cargarmultiple(asientos, cant, f-1, posc, 9);
            }
            else{
                asientos[f-1][posc] = 'r';
...
void cargarmultiple(char asientos[][9], int cant,int f, int posc, int cc){
    int counter = 1, countright = 0, countleft = 0, flag = 0;
    /*Check Right*/
    int i = posc + 1;
    while(i < cc && !flag && counter != cant){
        if(asientos[f][i]!= 'r'){
            counter++;
            countright++;
        }
    ...
    if(counter == cant){
        for(i = posc-countleft; i <posc+countright+1; i++){
            asientos[f][i] = 'r';
        }
    }
    else{
